...
def handleGraph0(date, radio, toggle):
    ...
    subplot_topgames = make_subplots(
        ...
        subplot_titles=('América do Norte', 'Europa', 'Japão', 'Outras Regiões')
    )
...
def handleGraph2(date, radio, toggle):
    ...
    fig_anos.add_annotation(
        text=f'Vendas em milhões de {date[0]} a {date[1]}',
        xref='paper',
        yref='paper',
        font=dict(
            size=20,
...
def handleIndicators(date, toggle):
    ...
    fig1.add_trace(
        go.Indicator(
            mode='number',
            title={'text': f"<span style='font-size:150%'>{name1} - Top Seller</span><br><span style='font-size:70%'>Em milhões de dólares</span><br><span style='font-size:7em>{date[0]} - {date[1]}</span>"},
            value= value1,
    ...
    fig2.add_trace(
        go.Indicator(
            mode='number',
            title={'text': f"<span style='font-size:150%'>{name2} - Top Genre</span><br><span style='font-size:70%'>Em milhões de dólares</span><br><span style='font-size:7em>{date[0]} - {date[1]}</span>"},
            value= value2,
            number= {'valueformat': '.2f'}
        )
    )
